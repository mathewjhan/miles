...
radix cache, pause modes) is assumed. Three of the four
(`extra_key` cache salting, in-place pause, abort by rid) are stock SGLang used
as-is; an extra feature, currently carried in an SGLang fork, is loading LoRA weights
over the distributed weight-update path with **upsert** semantics. Stock SGLang
can only load an adapter from disk under a fresh name and can only replace
weights by unload + reload; the sections below explain why neither works
...
```mermaid
flowchart TD
    subgraph CTRL["Controller (registry + proxy)"]
        REG["Registry<br/>pending / active / cleanup<br/>slot_versions"]
        PROXY["HTTP proxy<br/>(rid gate)"]
    end
    ...
    REG -->|"active_adapters"| DS
    DS -->|"sample tagged (name, slot=3) + reward spec"| WORKER
    WORKER -->|"stamp slot_version=1<br/>POST {lora_path: __miles_slot_3,<br/>rid: name::uuid, extra_key: name:v1}"| PROXY
    PROXY -->|"active? forward"| POOL
    POOL --> RADIX
    POOL -->|"tokens + logprobs"| PROXY
    PROXY -->|"reward scored, group done"| WORKER
...
> 4. The producer stamps `v1` and sends through the proxy
...
4. **Adapters in batch ⊆ Active adapters** Reconcile runs before batch
   collection from the queue. Between the two, the active set changes in only two
   ways: it grows at this iteration's `update_weights`, which promotes exactly
   the pending adapters reconcile just loaded (so active never exceeds loaded;
   adapters registered after the snapshot stay pending until the next iteration),
   and it shrinks on deregistration, which can happen at any time. Batch
   collection filters against the current active set, so the batch only contains
   active, and therefore loaded, adapters.

   *Example:* `math` deregisters while a batch is being collected. Its groups
   already in the batch still train correctly, because cleanup only happens at
   the *next* iteration's reconcile; the slot is untouched until then.
...
8. **Rid isolation.** `rid = name::uuid` with `::` banned from valid adapter names, so
   per-adapter abort by exact rid cannot affect other adapters.

   *Example:* retiring `code` aborts `code::7f3a…` but cannot touch
   `code_assist::91b2…`: aborts match whole rids, and names cannot contain `::`,
   so one tenant's retirement can never degrade a neighbor.
...
11. **Deregistered adapters cannot leak into training.** When we deregister an adapter, we need to
    close off three parts: new requests, in-flight requests not received by SGLang, and queued/processing requests in SGLang. We have three independent gates to address this: the proxy blocks new forward requests for that adapter, in-flight rids are aborted, and batch collection drops groups whose adapter is gone.

    *Example:* `math` is deregistered while one of its requests is mid-decode.
    New `math` requests get an instant dummy at the proxy; the in-flight one is
    aborted on the engine; if its response races the abort and arrives complete
    anyway, the proxy swaps it for a dummy on the way out. Three chances to catch
    it, any one sufficient.
...
sequenceDiagram
    ...
    par producer keeps filling the queue
        R->>C: active_adapters() (round-robin sampling)
        R->>E: POST /generate via proxy<br/>{lora_path: __miles_slot_i, rid, extra_key: name:vK}
    ...
    D->>C: mark_batch_trained(), per-adapter step += 1
...
**4. Abort by exact request id** (`abort_request` with `rid`, a stock feature).
When an adapter is deregistered, its in-flight requests must be reclaimed. Every
rollout request
carries `rid = "{adapter_name}::{uuid}"`, and retirement aborts exactly those
rids on every worker. The `::` separator is banned from adapter names so an
abort can never match another adapter's requests.

*Example:* deregistering `code` aborts `code::7f3a…` and `code::c410…`, and
`code_assist::91b2…` keeps running; `code::` can never prefix another
adapter's rids because names cannot contain `::`.

*Counter-example:* if aborts
matched on bare name prefixes (or names could contain the separator), retiring
`code` would also kill `code_assist`'s in-flight work: a deregistration of one
tenant silently degrading a healthy neighbor.

Everything below explains the trainer-side system built on top of these four.

## Components

### Controller: adapter registry + request gate
...
which version". Its registry is deliberately not a state machine, just three dicts
and a free-slot set:

- `pending`: slot claimed, weights not yet pushed to engines. Not sampleable.
- `active`: weights pushed at least once. Sampleable.
- `cleanup`: deregistered; the record is held until the trainer saves a final
  checkpoint and frees the slot.

Promotion `pending → active` happens in exactly one place, `record_weight_update`,
called after a successful push, so "active" *means* "the engines have this
adapter's weights". The registry also owns the monotonic `slot_versions`.

The controller additionally runs an HTTP proxy that all rollout traffic flows
through on its way to the engines. The proxy is the enforcement point for
retirement (invariant 11): requests whose adapter is no longer active get an
immediate abort-shaped response instead of being forwarded, and responses that
complete after their adapter was deregistered are replaced with a dummy so
stragglers never reach training. (A separate control-plane listener serves
register/deregister/status to external callers.)
...
with its adapter's current version, send it through the controller proxy to the
engines, score the result, and put the finished group on a queue. This loop never
...
```
snapshot   = controller.snapshot()          # pending / active / cleanup, atomically
reconcile_adapters(snapshot)                # converge trainer slots to the snapshot
...
controller.mark_batch_trained()             # per-adapter step counters
...
in a trainer slot when `train` runs. The active set grows only at this loop's own
`update_weights` (promoting exactly what reconcile just loaded) and shrinks only
on deregistration, so nothing can become sampleable that this iteration did not
load.
